...
//            $result = $this->{$this->callFunction};
    ...
    {
        return (isset($_POST[$var]) ? sanitize_option($var, wp_unslash($_POST[$var])) : $default_value);
    }
        ...
        if (!empty($this->delUserId)) {
            ...
            if ($user) {
                ...
                if (false !== $result) {
                    $this->wpdb->query(
                        $this->wpdb->prepare(
                            "DELETE FROM {$this->wpdb->prefix}mobileassistant_push_settings WHERE `user_id` = %s",
                            $this->delUserId
                        )
                    );
                    $this->wpdb->query(
                        $this->wpdb->prepare(
                            "DELETE FROM {$this->wpdb->prefix}mobileassistant_session_keys WHERE `user_id` = %s",
                            $this->delUserId
                        )
                    );
...
//                Mobassistantconnector_Functions::delete_empty_accounts();
    ...
    private function macGetUserData()
    {
        ...
        $user = $this->wpdb->get_results($query, ARRAY_A);
    ...
    private function macGetDevices()
    {
        ...
        $statuses_db = mobassist_get_order_statuses();
        $statuses    = array();
    ...
    private function changeDeviceStatus()
    {
        ...
        $result = $this->wpdb->query($this->wpdb->prepare("UPDATE {$this->wpdb->prefix}mobileassistant_push_settings SET `status` = %d WHERE `setting_id` IN (%s)", $this->statusValue, $ids));
    ...
    private function deleteDevice()
    {
        ...
        $result = $this->wpdb->query(
            $this->wpdb->prepare(
                "DELETE FROM {$this->wpdb->prefix}mobileassistant_push_settings WHERE `setting_id` IN (%s)",
                $ids
            )
        );
//        Mobassistantconnector_Functions::delete_empty_devices();
