...
class Mobassistantconnector_Access
{
    ...
    public static function clear_old_data()
    {
        ...
        if ($date_clear_prev === false || ( $timestamp - (int) $date_clear_prev ) > self::MAX_LIFETIME) {
            $wpdb->query(
                $wpdb->prepare(
                    'DELETE FROM `' . $wpdb->prefix . self::TABLE_SESSION_KEYS . '` WHERE `date_added` < %s', $date
                )
            );
            $wpdb->query(
                $wpdb->prepare(
                    'DELETE FROM `' . $wpdb->prefix . self::TABLE_FAILED_LOGIN . '` WHERE `date_added` < %s', $date
                )
            );
            $wpdb->update(
                $wpdb->options,
                array('option_value' => $timestamp),
                array( 'option_name' => 'mobassistantconnector_cl_date' ), '%d', '%s'
            );
        }
    }
...
/*        if ( hash( self::HASH_ALGORITHM, $login_data['login'] . $login_data['pass'] ) == $hash ) {
    ...
    public static function check_session_key( $key, $user_id = false )
    {
        ...
        $db_key = $wpdb->get_var(
            $wpdb->prepare(
                'SELECT sk.`session_key` FROM `' . $wpdb->prefix . self::TABLE_SESSION_KEYS. '` AS sk
    ...
    private static function generate_session_key( $user_id )
    {
        ...
        $wpdb->insert(
            $wpdb->prefix . self::TABLE_SESSION_KEYS, array( 'session_key' => $key, 'user_id' => $user_id, 'date_added' => $date),
            array( '%s', '%s', '%s')
        );
    ...
    private static function get_session_key_by_user_id( $user_id )
    {
        ...
        $session_key = $wpdb->get_var(
            $wpdb->prepare(
                'SELECT `session_key` FROM `' . $wpdb->prefix . self::TABLE_SESSION_KEYS
    ...
    public static function get_user_id_by_session_key( $session_key )
    {
        ...
        $user_id = $wpdb->get_var(
            $wpdb->prepare(
                'SELECT `user_id` FROM `' . $wpdb->prefix . self::TABLE_SESSION_KEYS
    ...
    public static function add_failed_attempt()
    {
        ...
        $timestamp = time();
        $wpdb->insert(
            $wpdb->prefix . self::TABLE_FAILED_LOGIN,
            array( 'ip' => sanitize_text_field(wp_unslash($_SERVER['REMOTE_ADDR'])), 'date_added' => date('Y-m-d H:i:s', $timestamp)),
            array( '%s', '%s' )
        );

        // Get count of failed attempts for last 24 hours and set delay
        $count_failed_attempts = $wpdb->get_var(
            $wpdb->prepare(
                'SELECT COUNT(`id`) FROM `' . $wpdb->prefix . self::TABLE_FAILED_LOGIN
            . '` WHERE `ip` = %s AND `date_added` > %s', sanitize_text_field(wp_unslash($_SERVER['REMOTE_ADDR'])), date('Y-m-d H:i:s', ($timestamp - self::MAX_LIFETIME))
            )
        );
    ...
    public static function check_auth( $hash, $logger = null )
    {
        ...
        if ($logger) {
            self::mobassist_log_me("Hash accepted is incorrect");
        }
    ...
    public static function get_allowed_actions_by_user_id( $user_id )
    {
        ...
        $actions = $wpdb->get_var(
            $wpdb->prepare(
                'SELECT `allowed_actions` FROM `' . $wpdb->prefix . self::TABLE_USERS
    ...
    public static function mobassist_log_me($message)
    {
        if (WP_DEBUG === true) {
