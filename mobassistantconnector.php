 ...
 * Plugin URI: https://emagicone.com/products/mobile-assistant-for-woocommerce/
...
/*-----------------------------------------------------------------------------+
| eMagicOne                                                                    |
| Copyright (c) 2022 eMagicOne.com <contact@emagicone.com>		               |
| All rights reserved                                                          |
+------------------------------------------------------------------------------+
|                                                                              |
 ...
 */
function mobileassistantconnector_check_is_woocommerce_activated()
{
...
if (mobileassistantconnector_check_is_woocommerce_activated()) {
    if (!class_exists('ma_connector')) {
        ...
        include_once 'classes/class-mobassistantconnector-exception.php';
        include_once 'functions/ema-functions.php';
        include_once 'option.php';
        include_once 'samain.php';
        include_once 'sa.php';
        include_once 'sanew.php';
        ...
        define('PUSH_TYPE_CHANGE_ORDER_STATUS', 'order_status_changed');
        ...
        class ma_connector
        {
            ...
            {
                load_plugin_textdomain('mobile-assistant-connector', false, basename(__DIR__) . '/i18n');
            }
            ...
            private function checkDBFields()
            {
                ...
                $is_exists_column = $wpdb->get_results(
                    $wpdb->prepare(
                        "SHOW COLUMNS FROM `{$wpdb->prefix}mobileassistant_push_settings` WHERE FIELD = %s", 'app_connection_id'
                    )
                );
                if ($is_exists_column) {
                    $wpdb->query(
                        "ALTER TABLE `{$wpdb->prefix}mobileassistant_push_settings` MODIFY app_connection_id VARCHAR (255) COLLATE utf8_unicode_ci"
                    );
                }
...
//                }
                ...
                $is_exists_column = $wpdb->get_results(
                    $wpdb->prepare(
                        "SHOW COLUMNS FROM `{$wpdb->prefix}mobileassistant_push_settings` WHERE FIELD = %s", 'not_notified_order_statuses_ids'
                    )
                );
                if (!$is_exists_column) {
                    $wpdb->query(
                        "ALTER TABLE `{$wpdb->prefix}mobileassistant_push_settings` ADD COLUMN `not_notified_order_statuses_ids` TEXT"
                    );
                }
            }
            ...
            private function check_db()
            {
                ...
                $wpdb->query(
                    ...
                    CREATE TABLE IF NOT EXISTS `{$wpdb->prefix}mobileassistant_push_settings` (
                        ...
                        `user_id` INT(10),
                        `app_connection_id` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
                        `push_new_order` tinyint(1) NOT NULL DEFAULT '0',
                        `push_order_statuses` text COLLATE utf8_unicode_ci NOT NULL,
                        `not_notified_order_statuses_ids` text COLLATE utf8_unicode_ci NOT NULL,
...
//                );
                        ...
                        if ($user = get_option('mobassistantconnector')) {
                            ...
                            $wpdb->query(
                                $wpdb->prepare(
                                  ...
                                  VALUES ('mobassistantconnector', %s, 'yes') ",
                                    serialize(array('mobassist_api_key' => $user['mobassist_api_key']))
                                )
                            );
            ...
            private function checkAndUpdateToNewVersion()
            {
                ...
                if (!$version || version_compare($version, MobileAssistantConnectorMain::PLUGIN_VERSION, '<')) {
                    $wpdb->replace(
                        $wpdb->options,
                        array(
                            'option_name' => self::OPTION_NAME_VERSION,
                            'option_value' => MobileAssistantConnectorMain::PLUGIN_VERSION
                        )
                    );
                }
                        ...
                        if ($has_user_all_actions) {
                            $wpdb->query(
                                $wpdb->prepare(
                                    ...
                                    '$action_codes_all_str' WHERE `user_id` = %s", $users[$i]['user_id']
                                )
                            );
                        }
                    }
...
//            }
            ...
            public function the_template($template)
            {
                ...
                if ($wp_query->query['connector'] == 'mobileassistant_new') {
                    $this->execute_connector_new();
                    exit;
                }
            ...
            public function execute_connector_new()
            {
                $MainClass = new MobileAssistantConnectorNew();
            ...
            public function ema_option_styles()
            {
                wp_register_style(
                    'ema_style',
                    plugins_url('css/style.css?ema_version=' . MobileAssistantConnectorMain::PLUGIN_VERSION, __FILE__)
                );
                ...
                wp_register_style(
                    'ema_style_bootstrap',
                    plugins_url(
                        'css/bootstrap.min.css?ema_version=' . MobileAssistantConnectorMain::PLUGIN_VERSION,
                        __FILE__
                    )
                );
            ...
            public function ema_option_scripts($hook)
            {
                if ($hook == 'toplevel_page_connector') {
                   wp_register_script(
                        'ema_qr',
                        plugins_url(
                            'js/qrcode.min.js?ema_version=' . MobileAssistantConnectorMain::PLUGIN_VERSION,
                            __FILE__
                        )
                    );
                    ...
                    wp_register_script(
                        'ema_qr_changed',
                        plugins_url(
                            'js/qr_changed.js?ema_version=' . MobileAssistantConnectorMain::PLUGIN_VERSION,
                            __FILE__
                        )
                    );
                    ...
                    if (!function_exists('curl_version')) {
                        $system_warnings .= __(
                            'Please enable cURL library to use Push Notifications feature!',
                            'mobile-assistant-connector'
                        );
                    }
                    ...
                    wp_register_script(
                        'ema_common',
                        plugins_url(
                            'js/ema_common.js?ema_version=' . MobileAssistantConnectorMain::PLUGIN_VERSION,
                            __FILE__
                        )
                    );
                    wp_localize_script(
            ...
            public function enqueue_bootstrap_script()
            {
                ...
                foreach ($wp_scripts->registered as $script) {
                    if ((stristr($script->src, 'bootstrap.min.js') !== false
                            or stristr($script->src, 'bootstrap.js') != false)
                        and wp_script_is($script->handle, $list = 'enqueued')
                    ) {
                ...
                if (!$bootstrap_enqueued) {
                    wp_register_script(
                        'ema_bootstrap',
                        plugins_url(
                            'js/bootstrap.min.js?ema_version=' . MobileAssistantConnectorMain::PLUGIN_VERSION,
                            __FILE__
                        )
                    );
...
if (!function_exists('connector_admin_notices')) {
    function connector_admin_notices()
    {
        ...
        echo '<b> Mobile Assistant Connector </b> add-on requires <a href="https://woocommerce.com/"> WooCommerce </a> plugin. Please install and activate it.';
        echo '</p></div>', "\n";
    }
}
?>