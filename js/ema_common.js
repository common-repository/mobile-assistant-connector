...
$jQema(document).ready(function() {
    ...
    $jQema('#users').on('click', 'li', function() {
    ...
    function setEvents() {
        $jQema('body').on('click', 'div.mobassistantconnector_status', function() {
            changeStatus($jQema(this));
        });

        $jQema('body').on('click', 'div.mobassistantconnector_delete', function() {
            if (confirm('Are you sure you want to delete device?')) {
                deleteDeviceBulk($jQema(this).attr('device_id'));
            }
        });

        $jQema('body').on('click', '#mobassistantconnector_bulk_select_all', function() {
            $jQema('.mobassistantconnector_pushes').each(function() {
                $jQema(this).prop('checked', true);
            });
        });

        $jQema('body').on('click', '#mobassistantconnector_bulk_unselect_all', function() {
            $jQema('.mobassistantconnector_pushes').each(function() {
                $jQema(this).prop('checked', false);
            });
        });

        $jQema('body').on('click', '#mobassistantconnector_bulk_status_active', function() {
            changeStatusBulk(1);
        });

        $jQema('body').on('click', '#mobassistantconnector_bulk_status_inactive', function() {
            changeStatusBulk(0);
        });

        $jQema('body').on('click', '#mobassistantconnector_bulk_delete', function() {
            if (confirm('Are you sure you want to delete selected devices?')) {
                deleteDeviceBulk(false);
            }
        });

        $jQema('body').on('submit', '#delete_user', function(event) {
            var form = $jQema(this);
        ...
        $jQema('body').on('submit', '#add_new_user', function(event) {
            var form = $jQema(this);
            var target = form.attr('data-target');
        ...
        $jQema('body').on('submit', '#form_mobassist', function(event) {
            var form = $jQema(this);
            var target = form.attr('data-target');
            ...
            function (data) {
                ...
                if (data != 'error' && data != 'Authentication error') {
                    ...
                    $jQema.each(data, function(index, values) {
                        ...
                        href_data += '<li id="'+values.user_id+'" class="col-sm-12 nav-item"><a class = "nav-link'
                        + active_class + '" href="#tab-user' +
                    ...
                    target.html(href_data + '<li id="user-add-ema" class = "col-sm-12 nav-item" style="cursor:pointer;"  data-toggle="modal" data-target="#emoModalNewUser"><a class="nav-link"><i class="fa fa-plus-circle" style="color:green"><img src="' + baseUrl + 'images/add4.png" title="Add user"></i> <span style="color:green;">&nbspAdd user </span></a></li>');
